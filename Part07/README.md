...
#### 함수의 형태 : 
- func 함수명(파라미터: 아규먼트) -> 아웃풋 { 실행문 } 형식으로 사용됩니다. 
- 함수는 단순하게 반복할 코드를 함수로 정의하여 사용할 수 있습니다. 
```Swift
func sringRepeat() {      
  print("반복1")
  print("반복2")  
...
- 리턴 타입이 없는 함수는 실제로 -> Void타입 으로 보통 ->()가 생략되어 사용됩니다.
```Swift
...
- 함수명(아규먼트레이블 파라미터: 아규먼트) 형식으로 작성해 사용합니다. 
```Swift
...
# 함수 실행의 메모리 구조 - 2 
- 스택은 스택끼리 메모리 공간을 공유 하지 않는다. 즉, 다른 스택의 변수를 찾아쓸수 없다는 Scope에 대한 이야기. 
