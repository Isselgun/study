...
이때, 변수나 상수가 전역변수라면 main() 스택 프레임이 아닌 데이터 영역에 저장이 됩니다.
이때, 함수의 실행문을 만나게 되면 메모리 스택 영역에서 main() 함수 영역 위에 층을 쌓아서 올려 새로운 스택 프레임 영역 생성합니다.
...
이때, 함수의 실행문을 만나게 되면 함수 스택 프레임 위에 층을 쌓아서 올려 새로운 함수 스택 프레임 영역 생성합니다. (반복 가능)
- 함수의 동작이 끝나면, 리턴값과 CPU의 제어권이 복귀주소를 통해 반환되며 스택 프레임이 사라집니다.
- 다시 한줄씩 CPU가 실행시킵니다. 

#### 함수 실행의 메모리 구조 - 2 
- 스택은 스택끼리 메모리 공간을 공유 하지 않습니다. 즉, 다른 스택의 변수를 찾아쓸수 없다는 Scope에 대한 이야기. 
- 전역변수는 메모리의 데이터 영역에 저장됩니다. 메모리의 데이터 영역과 힙 영역은 공유하는 영역이라서, Scope 범위 무관하게 접근 가능합니다. 


# 조건문과 반복문의 명령어(CPU) 구조 
#### 조건문과 반복문의 명령어 구조 : 
- 반복문이나 조건문은 스택 프레임을 만드는 행위가 아닙니다. 스택 프레임을 만드는 것은 반드시 함수의 실행에만 생성됩니다. 
- cpu가 줄을 이동하거나 뒤로 돌아가면서 동작합니다. 

# 입출력(inout) 파라미터 
#### 입출력 파라미터란 : 
- 함수를 통해, 변수를 직접 수정하고 싶은 경우 사용합니다. 함수내의 파라미터는 임시상수이기 때문에 변경 불가 원칙이기에 사용합니다. 
- inout을 사용 하면 복사되서 전달되는 값타입의 임시상수가 아닌 주소값이 전달 됩니다. 
(내부적으로 copy-in, copy-out 메모리 모델를 사용하지만, 실제 원본이 전달된다고 쉽게 생각하면 됨)
```Swift
func swapNumbers(a: inout Int, b: inout Int) {  // inout을 꼭 붙여야합니다 ⭐
    var temp = a
    a = b
    b = temp
}

swapNumbers(a: &num1, b: &num2)   // 앰퍼샌드를 꼭 붙여야합니다 ⭐ 
```
#### 주의점 : 
- 상수(let)나, 리터럴 전달하는 것 불가능합니다. 주소를 전달해야 하기 때문에, 값 자체를 직접적으로 전달하는게 안되기 때문입니다.
- 파라미터의 기본값 선언을 허용하지 않습니다. 값의 기본값을 선언하면 값을 바꿀수 없기 때문입니다.
- 가변파라미터(여러개의 파라미터)로 선언하는 것 불가능 합니다.

# guard문 
#### 가드문(guard statement): 
- 불만족하는 조건을 사전에 걸러내는 조건문입니다. else문을 먼저 배치하여 조건을 먼저 판별하여 조기 종료(early exit)시킵니다. 
- 가드문 사용 이유
1. 가독성을 높임. if문의 불편함을 해소
2. 여러 개의 옵셔널타입을 안전하게 벗기기 가능
  1) code의 영역이 변경되지 않음 (범위(Scope)에 대한 원칙에서 벗어나서 자유로운 코드 작성). 
  2) 가드문에서 선언된 변수를 아래문장에서 사용 가능 (동일한 스코프로 취급) (guard let 바인딩 관련)

#### 가드문 사용 방법 : 
- else문을 먼저 배치하여 조건을 만족하지 않을경우 else { 실행문 }의 형식으로 사용합니다.  
이때, else { 실행문 }의 실행문에는 조기 종료를 위해 제어전송문을 사용합니다. 함수에서는 return, throw, 반복문에서는 break, continue
- 조건을 만족하는 경우 코드가 다음 줄로 넘어가서 계속 실행합니다.
```swift
...
#### 함수의 리턴값과 CPU : 
- 함수의 리턴값이 있는 경우는 함수 실행시 반환하기 위한 임시 메모리 공간을 만듭니다. 없을경우 만들지 않습니다. 
- 함수의 리턴값이 있는 경우는 함수 실행시 CPU제어권과 리턴값이 함께 움직이며 없을경우 CPU 제어권만 움직입니다.


#### @어트리뷰트 키워드 : 
- 컴파일러에게 추가적인 정보를 제공하는 키워드를 의미합니다.
- 앞에 골뱅이@ 가 붙어있습니다. 
- 어트리뷰트는 아래와 같은 이유로 사용합니다. 
1) 선언에 추가 정보를 제공. ex. 클래스 앞에 @available 붙임.
2) 타입에 추가 정보를 제공. ex. 함수 타입 앞에 @escaping 키워드 붙임

#### 함수의 리턴값과 @discardableResult : 
- 리턴값이 있는 함수에서 리턴값을 사용하지 않는다면 함수의 결과값을 사용해야 한다는 경고창이 뜹니다. 경고창을 없애기 위해 @discardableResult을 사용합니다.
- 기존 방식(스위프트 5.2이전)은 아래와 같습니다.
```Swift
// 와일드카드 패턴을 사용
// 언더바에 함수값 할당하여 경고창 없앰
_ = sayHelloString()   
```               
- 스위프트 5.2부터의 방식은 아래와 같습니다. 
```Swift
// @discardableResult 어트리뷰트 키워드 이용
// 함수를 선언할 때, 함수 이름 앞에 붙이며, 결과값을 사용하지 않아도 된다고 컴파일러에게 정보를 알려줍니다.
@discardableResult
...
#### 함수와 연관지어서 튜플을 사용하는 이유 : 
- 함수는 한 개의 리턴값만 갖는 게 원칙입니다. 여러 개의 값을 반환하고 싶을 때는 여러 개의 값을 튜플로 묶어서 사용하고 반환합니다.
- 여러 개 반환 방법으로 튜플만 사용할 수 있는 건 아니며, (이후에 배움) 데이터 바구니를 사용하여 여러 리턴값을 가질수 있습니다.
