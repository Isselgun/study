...
# 함수 실행의 메모리 구조 - 1 
#### 컴퓨터의 기본 동작원리 : 
- 하드디스크에 있다가 실행시키면 램(메모리)위에 복사되어서 올라갑니다. 
- 램(메모리)는 코드, 데이터, 힙, 스택으로 공간이 있습니다. 
- 코드영역에는 코드들이 명령어 형태로 변경되어 저장이 됩니다.
- 코드영역에 있는 것들이 한줄씩 CPU에게 명령어를 내리고 처리됩니다.

#### 함수 실행의 메모리 구조 : 
- 명령어의 영역에서 함수가 있는 영역이 존재하고 코드를 순서대로 실행하다가 함수 실행문을 만나게 되면 함수의 특정 메모리 주소를 찾아가서 실행 시킵니다.  

#### addTwoNumbers(a: Int, b: Int) 함수의 실행 예제 : 
>가정 메모리 구조를 더 잘 이해하기 위하여 플레이그라운드에 대한 가정은 아래와 같습니다. 
>> main() 함수는 눈에 보이지 않지만, 구현이 되어 있다고 가정하자.
>> 여기에서 보이는 영역들은 메인 함수의 영역이라고 가정.
>> main() 함수는, 프로그램 시작했을 때 cpu가 가장 먼저 찾아가는 곳
1. 프로그래머의 코드를 컴파일된 형태로 바꿉니다. 
이때, 함수는 MAIN 영역 밖에 선언됩니다. 
2. MAIN으로 찾아가 CPU가 한줄씩 일을 합니다.
3. 함수를 만나게 된다면, 스택영역에 함수 스택을 만들고 제일 먼저 함수의 스택에 복귀주소를 저장해 둡니다. 함수를 다 실행하고 돌아와야하기 때문입니다.
4. 리턴 타입이 있다면 리턴 
5. 함수의 메모리 주소를 찾아가서 실행 시킵니다.
6. 함수를 다 실행하게 되면 함수의 리턴값이 있을경우 리턴값과 함께 CPU가 제어권을 찾아 원래의 주소로 돌아갑니다. 
7. 다시 한줄씩 CPU가 실행시킵니다. 

- 스택 : 
스택이란 아래에서 부터 쌓이고 , 제거 할때는 위에서부터 제거할 수 있는 구조. 반대편이 뚫려 있지 않은 통


---

- 메모리 영역
    - 코드: 앱 실행 시, 모든 코드가, 컴파일된 형태로 코드 영역에 올라감. 
    순차적으로 한줄씩 실행.
    - 데이터:
    - 힙
    - 스택: 함수 실행 시, 함수 실행에 필요한 메모리 공간을 만들고(스택프레임)
    함수가 실행이 완료되면 스택프레임은 사라진다.
    차곡차곡 아래에서부터 쌓이고, 제거할 때는 위에서부터 제거할 수 있는 구조.

- 프로그램 실행 과정
    - 프로그램이 실행되면, cpu는 main()에 가서 실행 시작
    - main() 함수 스택 프레임이 메모리 스택 영역에 생성된다.
    - main() 함수에서 변수나 상수 선언될 때마다, main() 함수 스택프레임에 메모리 공간 생성되고 값 저장됨
    - cpu가 코드 내려가다가 함수 실행
    - 메모리 스택 영역에서, main() 함수 영역 위에 층을 쌓아서 올려서,
    새로운 스택 프레임 영역 생성
    - cpu는 함수가 정의된 코드를 찾아가면, 먼저, 명령어의 복귀 주소를 저장
    - 함수가 리턴값이 있다면, 스택 프레임 내부에 리턴값을 저장할 임시 공간을 생성한다
    - 함수가 리턴값이 없다면, 임시 공간 생성X
    - 함수에서 파라미터가 선언되면 스택 프레임 내부에 파라미터 공간 생성
    - 함수 리턴값을 임시 공간에 저장
    - 함수의 동작이 끝나면, 스택 프레임이 사라지면서, 임시공간에 있던 리턴값이 반환됨.
    - main() 함수에 돌아옴. 복귀 주소 이용.
    cpu의 제어권과 리턴 값이 돌아온다.
    - 프로그램이 다 종료되면 main() 함수의 스택 프레임이 사라짐
